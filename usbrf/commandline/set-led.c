...
int                 cnt, vid, pid, isOn;
	...
	cnt = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, USBRF_RQ_GET_STATUS, 0, 0, buffer, sizeof(buffer), 5000);
	if(cnt < 1){
	    if(cnt < 0){
    ...
    */
	...
	uint32_t code = (978404 << 4) | 6 | (1 << 3);
	cnt = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
			      USBRF_RQ_SEND_CODE, (code >> 16), (code & 0xffff), buffer, sizeof(buffer), 5000);
	if (cnt < 0) {
	    fprintf(stderr, "USB error: %s\n", usb_strerror());
	}
	else {
        ...
        for(i = 0; i < 50000; i++){
            ...
            cnt = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, USBRF_RQ_ECHO, value, index, buffer, sizeof(buffer), 5000);
            if(cnt < 0){
